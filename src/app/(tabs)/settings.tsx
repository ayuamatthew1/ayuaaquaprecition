...
import FarmModal, {
  FarmFormData,
} from "@/src/components/settings/FarmModal";
import DeviceModal, {
  Device,
} from "@/src/components/settings/DeviceModal";

type Farm = FarmFormData & {
  id: string;
  status?: string;
};
...
export default function SettingsScreen() {
  ...
  const [deviceModalVisible, setDeviceModalVisible] =
    useState(false);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [
        ...
        authenticatedFetch("/api/farms"),
        authenticatedFetch("/api/devices/user-devices"),
      ]);

      const farmResult: ApiResponse<Farm[]> =
        await farmResponse.json().catch(() => ({
          success: false,
          message: "Invalid farm response.",
        }));

      const deviceResult: ApiResponse<{
        devices: Device[];
  ...
  const handleSaveFarm = async (
    ...
    Alert.alert(
      "Coming Soon",
      "Farm editing endpoint has not been implemented yet."
    );
  ...
  const handleRemoveDevice = async (
    ...
    Alert.alert(
      "Coming Soon",
      `Removing ${device.name} has not been implemented yet.`
    );
  };
  ...
  const handleConnectDevice = () => {
    Alert.alert(
      "Coming Soon",
      "Device pairing will be added after the device management API is completed."
    );
  };
      ...
      >
        ...
        <View style={styles.section}>
          <SettingRow
            ...
            title="Help & Support"
            description="Get help using Ayua Aquaprecition"
            onPress={() =>
              Alert.alert(
                "Help & Support",
          ...
          <SettingRow
            ...
            title="About"
            description="Ayua Aquaprecition v1.0.0"
          />
        </View>
